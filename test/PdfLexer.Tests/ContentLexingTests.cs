...
f";
            ...
            var cm = (cm_Op<double>)op;
            Assert.Equal(0.06, cm.a);
            Assert.Equal(0, cm.b);
            Assert.Equal(0, cm.c);
            Assert.Equal(-0.06, cm.d);
            Assert.Equal(1, cm.e);
            Assert.Equal(594, cm.f);
            scanner.SkipCurrent();
            Assert.Equal(PdfOperatorType.re, scanner.Peek());
            Assert.Equal(4, scanner.Operands.Count);
            op = scanner.GetCurrentOperation();
            Assert.Equal(PdfOperatorType.re, op.Type);
            var re = (re_Op<double>)op;
            Assert.Equal(870, re.x);
            Assert.Equal(1095, re.y);
            Assert.Equal(768, re.width);
            Assert.Equal(176, re.height);
            scanner.SkipCurrent();
...
EI";
            ...
            Assert.Equal(PdfOperatorType.BI, scanner.Peek());
            var op = scanner.GetCurrentOperation();
            var opImage = op as InlineImage_Op<double>;
...
Q";
            ...
            var op = (DP_Op<double>)scanner.GetCurrentOperation();
