...
namespace PdfLexer.Objects.Tests
{
    public class DetectionTests
    {
        ...
        [Theory]
        public void It_Detects_Objects(string input, bool success, int startPos, int expectedLength, bool completed, PdfTokenType type)
        {
            ...
            var result = reader.TryReadNextToken(completed, out var tokenType, out var start, out var length);
            if (success)
            {
                var data = sequence.Slice(start, length);
                Assert.True(result);
