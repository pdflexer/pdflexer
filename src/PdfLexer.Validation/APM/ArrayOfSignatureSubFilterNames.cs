...
internal partial class APM_ArrayOfSignatureSubFilterNames : ISpecification<PdfArray>
{
    ...
    {
        var c = ctx.Clone();
        c.Run<APM_ArrayOfSignatureSubFilterNames_x, PdfArray>(new CallStack(), obj, null);
        if (c.Errors.Any())
        {
            return false;
        }
...
internal partial class APM_ArrayOfSignatureSubFilterNames_x : ISpecification<PdfArray>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfArray obj, IPdfObject? parent)
    {
        ...
        {
            var (val, wasIR) = ctx.GetOptional<PdfName, APM_ArrayOfSignatureSubFilterNames_x>(obj, n, IndirectRequirement.Either);
            if (val == null) { return; }
            // no special cases
            
            
            if (!(val == PdfName.adbepkcs7detached || (ctx.Version < 2.0m && val == PdfName.adbepkcs7sha1) || val == PdfName.adbex509rsa_sha1 || (ctx.Version >= 2.0m && val == PdfName.ETSICAdESdetached) || (ctx.Version >= 2.0m && val == PdfName.ETSIRFC3161))) 
            {
