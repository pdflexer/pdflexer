...
internal partial class APM_DestXYZStructArray : ISpecification<PdfArray>
{
    ...
    {
        var c = ctx.Clone();
        c.Run<APM_DestXYZStructArray_1, PdfArray>(new CallStack(), obj, null);
        if (c.Errors.Any())
        {
            return false;
        }
...
internal partial class APM_DestXYZStructArray_1 : ISpecification<PdfArray>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfArray obj, IPdfObject? parent)
    {
        var (val, wasIR) = ctx.GetRequired<PdfName, APM_DestXYZStructArray_1>(obj, 1, IndirectRequirement.Either);
        if (val == null) { return; }
        // no special cases
