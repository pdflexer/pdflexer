...
internal partial class APM_ActionNOP_Type__Base : ISpecification<PdfDictionary>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfDictionary obj, IPdfObject? parent)
    {
        var (val, wasIR) = ctx.GetOptional<PdfName, APM_ActionNOP_Type>(obj, "Type", IndirectRequirement.Either);
        if (val == null) { return; }
        // no special cases
...
internal partial class APM_ActionNOP_S__Base : ISpecification<PdfDictionary>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfDictionary obj, IPdfObject? parent)
    {
        var (val, wasIR) = ctx.GetRequired<PdfName, APM_ActionNOP_S>(obj, "S", IndirectRequirement.Either);
        if (val == null) { return; }
        // no special cases
