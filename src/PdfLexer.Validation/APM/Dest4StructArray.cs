...
internal partial class APM_Dest4StructArray : ISpecification<PdfArray>
{
    ...
    {
        var c = ctx.Clone();
        c.Run<APM_Dest4StructArray_1, PdfArray>(new CallStack(), obj, null);
        if (c.Errors.Any())
        {
            return false;
        }
...
internal partial class APM_Dest4StructArray_1 : ISpecification<PdfArray>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfArray obj, IPdfObject? parent)
    {
        var (val, wasIR) = ctx.GetRequired<PdfName, APM_Dest4StructArray_1>(obj, 1, IndirectRequirement.Either);
        if (val == null) { return; }
        // no special cases
