...
internal partial class APM_ArrayOfNamesGeneral : ISpecification<PdfArray>
{
    ...
    {
        var c = ctx.Clone();
        c.Run<APM_ArrayOfNamesGeneral_x, PdfArray>(new CallStack(), obj, null);
        if (c.Errors.Any())
        {
            return false;
        }
...
internal partial class APM_ArrayOfNamesGeneral_x : ISpecification<PdfArray>
{
    ...
    public static void Validate(PdfValidator ctx, CallStack stack, PdfArray obj, IPdfObject? parent)
    {
        ...
        {
            var (val, wasIR) = ctx.GetOptional<PdfName, APM_ArrayOfNamesGeneral_x>(obj, n, IndirectRequirement.Either);
            if (val == null) { return; }
            // no special cases
