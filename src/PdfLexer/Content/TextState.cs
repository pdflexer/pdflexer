...
public class TextState
{
    ...
    public TextState(ParsingContext ctx, GraphicsState gs, PdfDictionary? pageResources)
    {
        Ctx = ctx;
        GS = gs;
        PageResources = pageResources ?? new PdfDictionary();
    ...
    public int GetGlyph(ReadOnlySpan<byte> data, int pos, out Glyph info)
    {
        if (GS.Font == null)
        {
            GS.Font = SingleByteFont.Fallback;
    ...
    public void Apply(Tf_Op op)
    {
        ...
        if (obj == null)
        {
            Ctx.Error($"Failed to get font {op.font}, falling back to helvetica.");
            GS.Font = SingleByteFont.Fallback;
        } else
        {
