...
internal class CalRGB : IColorSpace
{
    ...
    public static CalRGB FromObject(PdfDictionary dict)
    {
        ...
        if (ga != null)
        {
            var gav = ga.Select(x => x.GetAsOrNull<PdfNumber>()).Where(x => x != null).Select(x => (float)x!).ToList();
            if (gav.Count == 3)
            {
        ...
        if (m != null)
        {
            var mv = m.Select(x => x.GetAsOrNull<PdfNumber>()).Where(x => x != null).Select(x => (float)x!).ToList();
            if (mv.Count == 9)
            {
                mxa = mv[0];
                mya = mv[1];
