...
namespace PdfLexer.Fonts
{
    internal class BCMapReader
    {
        ...
        private static CResult hexToStr(byte[] a, int size)
        {
            ...
            {
                return new CResult { MultiChar = UnicodeEncoding.Unicode.GetString(data.Slice(0, size)) };
            }
        }
        ...
        public static (List<CMapRange> Ranges, Dictionary<uint, CResult> Mapping) GetGlyphsFromToUnicode(ParsingContext ctx, ReadOnlySpan<byte> data)
        {
            ...
            for (var i = 0; i < data.Length; i++)
            {
                ...
                {
                    throw new PdfLexerException("BinaryCMapReader.process: Invalid dataSize.");
                }
                ...
                switch (type)
                {
                    ...
                    case 4: // bfchar
                        ...
                        mapping[hexToInt(c, ucs2DataSize)] = hexToStr(charCode, dataSize);
                        for (var k = 1; k < subitemsCount; k++)
                        {
                    ...
                    case 5: // bfrange
                        {
                            ...
                            while (low <= high)
                            {
                                ...
                                charCode[lb] = (byte)(charCode[lb] + 1);
                                // todo edge case
                            }
                            for (var k = 1; k < subitemsCount; k++)
                            {
                    ...
                    default:
                        throw new PdfLexerException($"BinaryCMapReader.process - unknown type: ${ type }");
                }
            }
